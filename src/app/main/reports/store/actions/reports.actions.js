...
export function getAdmissionReports(data) {
  return (dispatch, getState) => {
    ...
    console.log('getAdmissionReports', data);
...
export function getCremationReports(data) {
  return (dispatch, getState) => {
    ...
    console.log("getCremationReports", data);
...
export function getReleaseReports(data) {
  return (dispatch, getState) => {
    dispatch({ type: FETCH_PROGRESS });
    const user = getState().auth.user.data;
    ...
    if (data) {
      ...
      finalData = {
        ...
        orgKey: user.organisation.orgId,
        startDate: moment("1999-07-07").format("YYYY-MM-DDTHH:mm:ss"),
      };
    }

    const request = axios.post("/api/v1/reports/release-form", finalData);
